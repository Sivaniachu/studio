...
const initialLines: Line[] = [
  { id: "init-1", content: `CmdWeb [Version ${APP_VERSION}]`, type: "info" },
  { id: "init-2", content: "(c) Firebase Studio. All rights reserved.", type: "info" },
  { id: "init-3", content: <>&nbsp;</>, type: "info" },
];
...
export default function CmdWebPage() {
  ...
  const inputRef = useRef<HTMLInputElement>(null); // Hidden input for focus management
  ...
  const fetchSuggestions = useCallback(async (prefix: string) => {
    if (!prefix.trim() || prefix.includes(" ")) { // Only suggest for first part of command
      setSuggestions([]);
      return;
    }
  ...
  const processCommand = (commandStr: string) => {
    ...
    switch (command.toLowerCase()) {
      case "cls":
        setLines([{ id: generateLineId(), content: <>&nbsp;</>, type: "info" }]); // Keep one empty line for feel
        break;
      case "echo":
      ...
      case "time":
        ...
        break;
      case "ver":
        addLine(`CmdWeb [Version ${APP_VERSION}]`, "output");
        break;
      case "exit":
        addLine("Exiting CmdWeb... (This is a simulation)", "info");
        // In a real Electron app, this might close the window.
        break;
      case "": // Empty command
        break;
      default:
  ...
  const handleSubmit = () => {
    ...
    const displayInput = `${PROMPT_STRING}${currentInput}`;
    ...
    setCurrentInput("");
    setHistoryIndex(commandHistory.length); // Reset history index to new "end"
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      ...
      if (suggestions.length > 0) {
        ...
        setHistoryIndex(newIndex);
        setCurrentInput(commandHistory[newIndex] || "");
         if (newIndex === commandHistory.length) setCurrentInput(""); // Clear if past last item
      }
    } else if (e.key === "Tab") {
      e.preventDefault();
      if (suggestions.length > 0) {
        ...
        if (suggestionToApply) {
          setCurrentInput(suggestionToApply + " "); // Add space for next arg
          setSuggestions([]);
          setActiveSuggestionIndex(-1);
        }
      }
  ...
  return (
    <div 
      className="min-h-screen flex flex-col p-2 focus:outline-none bg-background text-foreground w-full h-full overflow-hidden"
      onClick={() => inputRef.current?.focus()} // Focus hidden input on click anywhere
      tabIndex={-1} // Make div focusable if needed, but input handles it
    >
      <div className="flex-grow overflow-y-auto pr-2"> {/* Added pr-2 for scrollbar space */}
        {lines.map((line) => (
          <div
            key={line.id}
            className={cn("whitespace-pre-wrap break-words", { // Allow long words to break
        ...
        {/* Current input line area */}
        <div className="flex items-center">
          <span className="text-cmd-prompt">{PROMPT_STRING}</span>
          {/* This span will visually represent the input, the actual input is hidden */}
      ...
      <input
        ...
        className="opacity-0 w-0 h-0 absolute -top-96 -left-96" // Visually hide but keep focusable
